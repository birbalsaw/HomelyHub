...
// importing CSS file for input styling
...
const FilterModal = ({ selectedFilters, onFilterChange, onClose }) => {
  ...
  // useEffect hook to update the states when selectedFilters prop changes
  useEffect(() => {
    setPriceRange({
      min: selectedFilters.priceRange?.min || 600,
      max: selectedFilters.priceRange?.max || 30000,
  ...
  // function to handle clearning filter
  const handleClearFilters = () => {
    setPriceRange({ min: 600, max: 30000 }); //reset the price
    setPropertyType("");
    setRoomType("");
    setAmenities([]);
  };
  ...
  // function to handle change in propertyType
